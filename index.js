...
var Markdown = {
    ...
    getConfig: function (config, callback) {
        config.markdown = {
            ...
            highlightLinesLanguageList:
                Markdown.config.highlightLinesLanguageList,
    ...
    getLinkTags: function (hookData, callback) {
        hookData.links.push({
            ...
            href:
                ...
                "/plugins/nodebb-plugin-ws-markdown/styles/" +
                (Markdown.config.highlightTheme || "railscasts.css"),
        });
    ...
    init: function () {
        ...
        meta.settings.get("markdown", function (err, options) {
            if (err) {
                winston.warn(
                    "[plugin/markdown] Unable to retrieve settings, assuming defaults: " +
                        err.message
                );
            }
            ...
            for (var field in defaults) {
                ...
                if (!options.hasOwnProperty(field)) {
                    ...
                    field !== "langPrefix" &&
                    field !== "highlightTheme" &&
                    field !== "headerPrefix" &&
                    field !== "highlightLinesLanguageList"
                ) {
            ...
            if (typeof _self.config.highlightLinesLanguageList === "string") {
                try {
                    _self.config.highlightLinesLanguageList = JSON.parse(
                        _self.config.highlightLinesLanguageList
                    );
                } catch (e) {
                    winston.warn(
                        "[plugins/markdown] Invalid config for highlightLinesLanguageList, blanking."
                    );
                    _self.config.highlightLinesLanguageList = [];
                }

                _self.config.highlightLinesLanguageList = _self.config.highlightLinesLanguageList
                    .join(",")
                    .split(",");
            }
    ...
    loadThemes: function () {
        fs.readdir(
            path.join(require.resolve("highlight.js"), "../../styles"),
            function (err, files) {
                if (err) {
                    winston.error(
                        "[plugin/markdown] Could not load Markdown themes: " +
                            err.message
                    );
                    Markdown.themes = [];
                    return;
            ...
            [
                function (next) {
                    if (
                        ...
                        data.postData.content = parser.render(
                            data.postData.content
                        );
                    }
                    next(null, data);
            ...
            [
                function (next) {
                    if (
                        ...
                        data.userData.signature = parser.render(
                            data.userData.signature
                        );
                    }
                    next(null, data);
        ...
        var formatting = [
            {
                ...
                title: "[[modules:composer.formatting.bold]]",
            },
            {
                name: "italic",
                className: "fa fa-italic",
                title: "[[modules:composer.formatting.italic]]",
            },
            {
                name: "list",
                className: "fa fa-list-ul",
                title: "[[modules:composer.formatting.list]]",
            },
            {
                name: "strikethrough",
                className: "fa fa-strikethrough",
                title: "[[modules:composer.formatting.strikethrough]]",
            },
            {
                name: "code",
                className: "fa fa-code",
                title: "[[modules:composer.formatting.code]]",
            },
            {
                name: "link",
                className: "fa fa-link",
                title: "[[modules:composer.formatting.link]]",
            },
            {
                name: "picture-o",
                className: "fa fa-picture-o",
                title: "[[modules:composer.formatting.picture]]",
            },
        ];
    ...
    updateParserRules: function (parser) {
        ...
        parser.renderer.rules.image = function (
            ...
            // Validate the url
            if (!Markdown.isUrlValid(tokens[idx].attrs[srcIdx][1])) {
                return "";
            }
            ...
            if (classIdx < 0) {
                ...
                tokens[idx].attrs[classIdx][1] +=
                    " img-responsive img-markdown";
            }

            return renderImage(tokens, idx, options, env, self);
        };

        parser.renderer.rules.link_open = function (
            tokens,
            idx,
            options,
            env,
            ...
            if (!Markdown.config.allowRTLO) {
                if (tokens[idx + 1] && tokens[idx + 1].type === "text") {
                    if (
                        tokens[idx + 1].content.match(
                            Markdown.regexes.rtl_override
                        )
                    ) {
                        tokens[idx + 1].content = tokens[
                            idx + 1
                        ].content.replace(Markdown.regexes.rtl_override, "");
                    }
                }
            }
    ...
    isUrlValid: function (src) {
        ...
        const allowed = (pathname) =>
            allowedRoots.some(
                (root) =>
                    pathname.toString().startsWith(root) ||
                    pathname
                        .toString()
                        .startsWith(nconf.get("relative_path") + root)
            );
    ...
    isExternalLink: function (urlString) {
        ...
        if (
            ...
            (urlObj.host === baseUrlObj.host &&
                ...
                (nconf.get("relative_path").length > 0
                    ? urlObj.pathname.indexOf(nconf.get("relative_path")) === 0
                    : true)) // Subfolder installs need this additional check
        ) {
            return false;
        }
        return true;
